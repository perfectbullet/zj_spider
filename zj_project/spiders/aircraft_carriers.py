...
class MySpider(scrapy.Spider):
    # aircraft_carriers
    name = "aircraft_carriersv3"
    allowed_domains = ['en.wikipedia.org']
    # english version
    start_urls = ['https://{}/wiki/Category:Aircraft_carriers'.format(allowed_domains[0])]
    ...
    def parse(self, response):
        ...
        for cl in category_urls:
            if re.search(r'List_of|Lists_of', cl) is None and re.search(r'Category', cl) is not None:
    ...
    def parse_category(self, response):
        ...
        for cl in content_urls + content_urlsv2:
            if (re.search('Category|Template|List_of|Lists_of', cl) is None
                    and re.search('carrier|Carrier', response.url)):
                # 当前分类 carrier
        ...
        for cl in new_categories:
            if (re.search('Category', cl) is not None and re.search('Aircraft_carriers', cl) is not None
                    and re.search('Lists_of|List_of|Airborne', cl) is None):
