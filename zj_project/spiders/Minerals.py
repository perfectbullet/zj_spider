...
class MySpider(scrapy.Spider):
    ...
    # english version
    start_urls = [
        'https://{}/wiki/Category:Minerals'.format(allowed_domains[0]),
        'https://{}/wiki/Category:Natural_materials'.format(allowed_domains[0])
    ]
    ...
    def parse(self, response):
        ...
        if not category_urls:
            category_urls = search.xpath("//div[@class='mw-category']//div[@class='CategoryTreeItem']//a/@href").extract()
        new_category_urls = []
        self.logger.info('parse category_urls is %s', category_urls)
        for cl in category_urls:
            if re.search(r'Mineral|Minerals|minerals|mineral', cl) and re.search(r'Category', cl):
    ...
    def parse_category(self, response):
        self.logger.info('parse_category response is {}, response.url {}'.format(response, response.url))
        search = response.xpath("//main[@id='content']")
        ...
        for cl in content_urls + content_urlsv2:
            if not re.search(':Category|Template|List_of|Lists_of|list_of|lists_of', cl):
                # 当前分类 carrier
        ...
        for cl in new_categories:
            if re.search('Category', cl) and re.search('Mineral|Minerals|minerals|mineral', cl):
